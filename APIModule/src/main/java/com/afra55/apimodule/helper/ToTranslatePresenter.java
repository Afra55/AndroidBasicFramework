...
import com.afra55.commontutils.http.ProgressRequestBody;
import com.afra55.commontutils.http.RequestBody;
import com.afra55.commontutils.http.RequestPart;
import com.afra55.commontutils.http.RequestQuery;
import com.afra55.commontutils.http.RxPresenter;
import com.afra55.commontutils.http.UploadProgressListener;
...
public class ToTranslatePresenter extends RxPresenter {
    ...
    public void upload(File file, UploadProgressListener listener) {
        addSubscriber(apiServices.upload(
                RequestQuery.getBuildInstance()
                        .build()
                , RequestPart.getBuildInstance()
                        .withParam("name", "text/plain", file)
                        .build()
                , ProgressRequestBody.createPartOfMultipartBody("image", "image/jpeg", file, listener))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new BaseDisposableObserver<String>() {
                    @Override
    ...
    public void toTranslate(String string) {
        ...
        addSubscriber(apiServices.toTranslate(
                RequestQuery.getBuildInstance()
                        .withParam("q", string)
                        .withParam("from", "auto")
                        .withParam("to", "auto")
                ...
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new BaseDisposableObserver<TranslateBean>() {
                    @Override
